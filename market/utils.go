...
func TokenPriceInUSD(token common.Address) (float64, error) {
	// 转为小写字符串地址
	_, price, err := fetchTokenPriceByContractHTTP(common.HexToAddress(token.Hex()).Hex())
	if err != nil {
		return 0, err
	}
...
func TokenContractAddressToCoinId(token common.Address) (string, error) {
	coinId, _, err := fetchTokenPriceByContractHTTP(common.HexToAddress(token.Hex()).Hex())
	if err != nil {
		return "", err
	}
	return coinId, nil
}
func GetHolderAddressesByChainID(chainId string) ([]string, error) {
	result, err := fetchTopHoldersAddressByChainIDHTTP(chainId)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func GetMonitTokenAddressesByChainID(chainId string) ([]string, error) {
	results, err := fetchMonitTokenAddressesByExchangeIDHTTP("binance_futures", chainId)
	if err != nil {
		return nil, err
	}
	return results, nil
